...
struct NodeInfo {
    int64_t id_;
    std::vector<int64_t> links_;

    NodeInfo() = default;

    explicit NodeInfo(const int64_t id, std::vector<int64_t>&& links) : id_(id), links_(std::move(links)) {
    }

    NLOHMANN_DEFINE_TYPE_INTRUSIVE(NodeInfo, id_, links_)
};
