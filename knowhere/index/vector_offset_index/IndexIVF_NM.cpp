...
IVF_NM::GetIndexMeta() {
    ...
    for (int32_t i = 0; i < nlist; i++) {
        ...
        sids.reset(new faiss::InvertedLists::ScopedIds(ivf_index->invlists, i));
    ...
    KNOWHERE_THROW_IF_NOT_FMT(cnt == ntotal, "invlists total size mismatch (%ld vs. %ld)", cnt, ntotal);
